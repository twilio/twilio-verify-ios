...
class KeychainQueryTests: XCTestCase {
  ...
  func testCreateQuery_shouldHaveGivenAttributes() {
      ...
      XCTAssertNotNil(query[kSecUseOperationPrompt])
      XCTAssertEqual(query[kSecAttrAccount] as? String, testKey)
      XCTAssertEqual(query[kSecUseOperationPrompt] as? String, authenticationPromptTestKey)
  }
}

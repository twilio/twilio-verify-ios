...
twilioVerify.updateChallenge(withPayload: payload, success: {
  ...
  if case .inputError(let detail) = error, let inputError = detail as? InputError {
    switch inputException {
      ...
      case .expiredChallengeException: return
      default: return
    }
  }
})
```
You can find the associated exceptions for validations [here](https://github.com/twilio/twilio-verify-ios/blob/main/TwilioVerifySDK/TwilioVerify/Sources/TwilioVerifyError.swift#L152)
